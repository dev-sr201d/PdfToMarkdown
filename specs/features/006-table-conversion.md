...
## Inputs
- Parsed PDF content with structural table/cell groupings and text content as produced by FRD-003.
...
2. The system must identify the header row of a table (typically the first row or a row with distinct formatting) and render it as the Markdown table header.
3. The system must render a separator row (`|---|---|...`) immediately after the header row, as required by Markdown table syntax.
4. The system must render each subsequent row as a Markdown table data row.
5. Cell content must be preserved accurately, including any inline text formatting (which may be further processed by FRD-007).
6. The system must handle tables with varying numbers of columns across rows gracefully — padding shorter rows with empty cells to maintain consistent column counts.
7. Tables must be placed in the output in their original reading-order position relative to surrounding content.
8. The system must handle cells containing multi-line text by collapsing it to single-line content within the cell (Markdown tables do not support multi-line cells).
...
## Dependencies
- **FRD-003** (PDF Parsing & Content Extraction) — requires structural table/cell groupings from the parser.
